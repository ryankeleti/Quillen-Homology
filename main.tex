...
Let $(\C/B)_\text{ab}$ be the category of abelian group objects in $\C/B$ and let
...
\]
\[
  (\C/B)_\text{ab}\simeq\M_B
\]
\[
  X\longmapsto\ker u_X.
\]
In particular $(\C/B)_\text{ab}$ is an abelian category.
\end{prop}

If $Y$ is an $A$-algebra, let $D_{Y/A}$ be the $Y$-module of $A$-differentials of $Y$.
There is a canonical $A$-derivation $d:Y\to D_{Y/A}$ such that
\[
  \Hom_{\M_Y}(D_{Y/A},N)\simeq\Der(Y/A,N)
\]
\[
...
\[  
  ...
  \simeq\Hom_{\M_Y}(D_{Y/A},M)\simeq\Hom_{\M_B}(D_{Y/A}\otimes_Y B,M).\tag{1.6}
\]
Thus
...
and let $\L D_{B/A}$ be the simplicial $B$-module $D_{P/A}\otimes_P B$ obtained by applying the
``abelianization'' functor $Y\mapsto D_{Y/A}\otimes_Y B$ dimension-wise to $P$. The simplicial
homotopy type of $\L D_{B/A}$ is independent of the choice of $P$ by 2.12. The underlying chain
complex of $\L D_{B/A}$ is called the cotangent complex of the $A$-algebra $B$ and is unique up
to chain homotopy equivalence of the choice of $P$. The following groups are then well-defined:
\[
  \D_q(B/A,M)=\H_q(\L D_{B/A}\otimes_B M),\tag{2.15}
\]
\[
  \D^q(B/A,M)=\H^q(\Hom_B(\L D_{B/A},M)).\tag{2.16}
\]
...
We have
\[
  \D^q(B/A,M)=\H^q(\Hom_B(D_{P/A}\otimes_P B,M))=\H^q(\Der(P/A,M)),
\]
...
\begin{prop}{3.1}
If $B$ is a free $A$-algebra, then $\L D_{B/A}\simeq cD_{B/A}$ and hence
\[
...
then $\{dx\otimes 1 : x\in C_q\}\subset D_{P_q/A}\otimes_{P_q}B$ is a set of generators as
in 3.2. Hence $D_{P/A}\otimes_P B$ is a free simplicial $B$-module. If $Q$ is a projective
$A$-algebra resolution, then by 2.9 $Q$ is a retract of a free one $P$ and so $D_{Q/A}\otimes_Q B$
is a projective simplicial $B$-module. In particular as a chain complex $D_{Q/A}\otimes_Q B$ is
projective in each dimension. Thus

\begin{prop}{3.3}
$\L D_{B/A}$ is a projective simplicial $B$-module.
\end{prop}
...
\begin{prop}{3.8}
$\D^0(B/A,M)=\Der(B/A,M)$, $\D_0(B/A,M)=D_{B/A}\otimes_B M$.
\end{prop}
\begin{proof}
As $Y\mapsto D_{Y/A}\otimes_Y B$ is a left adjoint functor by 1.6, it is
right exact. Hence $\D_0(B/A)=D_{B/A}$ and the proposition follows from 3.6 and 3.7.
\end{proof}
...
begin by recalling facts about simplicial modules which are proved in [HA, II, \textsection~6].
...
\[
  \L D_{S/R}\otimes_R R'\simeq\L D_{S'/R'},\tag{4.7}
\]
\[
  \L D_{S'/R}\simeq\L D_{S/R}\otimes_R R'\oplus\L D_{R'/R}\otimes_R S.\tag{4.8}
\]
\end{nenv}
\begin{proof}
...
\[
  \L D_{S'/R'}=D_{P'/R'}\otimes_{R'}S'\simeq(D_{P/R}\otimes_R S)\otimes_R R'\simeq\L D_{S/R}\otimes_R R'
\]
...
\begin{align*}
  \L D_{S'/R'}&=D_{P\otimes_R Q/R}\otimes_{P\otimes_R Q}S'\simeq(D_{P/R}\otimes_R Q\oplus D_{Q/R}\otimes_R P)\otimes_{P\otimes_R Q}S'\\
              &\simeq(D_{P/R}\otimes_P S)\otimes_R R'\oplus(D_{Q/R}\otimes_Q R')\otimes_R S\\
              &\simeq\L D_{S/R}\otimes_R R'\oplus\L D_{R'/R}\otimes_R S.
\end{align*}
...
$\Ho(\M_T)$
\[
  \L D_{S/R}\otimes_S T\lra\L D_{T/R}\lra\L D_{T/S}\lra\Sigma(\L D_{S/R}\otimes_S T).\tag{4.12}
\]
\end{nenv}
\begin{proof}
Form a diagram
...
$q=1$ and in [?] for $q=1,2$. We shall state these results only for the cotangent complex $\L D_{B/A}$ leaving
the translation for the functors $\D^q$ and $\D_q$ to the reader.

\begin{prop}{5.1}
If $S$ is a multiplicative system in $A$, then $\L D_{S^{-1}A/A}\simeq 0$.
\end{prop}
\begin{proof}
(after Andr{\'e} [?, 20.1]) Let $C=S^{-1}A$. As $C\otimes_A C\simeq C$ we have an isomorphism
$\L D_{C/A}\otimes_A C\simeq\L D_{C/A}$ of projective simplicial $C$-modules. As $C$ is flat over
$A$ the former complex by 4.7 is isomorphic to $\L D_{C\otimes_A C/C}\simeq\L D_{C/C}\simeq 0$.
\end{proof}
...
\[
  \L D_{T^{-1}B/S^{-1}A}\simeq\L D_{B/A}\otimes_B T^{-1}B.
\]
\end{env}
\begin{proof}
Applying 4.17 to $A\to B\to T^{-1}B$ and $A\to S^{-1}A\to T^{-1}B$ and using 5.1 we have
\[
  \L D_{B/A}\otimes_B T^{-1}B\simeq\L D_{T^{-1}B/A}\simeq\L D_{T^{-1}B/S^{-1}A}.
\]
\end{proof}
...
\begin{center}
  $B$ is {\'e}tale over $A\iff\L D_{B/A}\simeq 0$;
\end{center}
\begin{center}
  $B$ is smooth over $A\iff\L D_{B/A}\simeq cD_{B/A}$ and $D_{B/A}$ is a projective $B$-module.
\end{center}
\end{prop}
\begin{proof}
...
\begin{align*}
  (\L D_{B/A})_{\mathfrak{p}}&\simeq(\L D_{B/A}\otimes_A B)_{\mathfrak{q}}\\
                             &\simeq(\L D_{B\otimes_A B/B})_{\mathfrak{q}}\quad\text{by 4.9 since $B$ is flat over $A$.}\\
                             &\simeq\L D_{(B\otimes_A B)_{\mathfrak{q}}/B_{\mathfrak{p}}}\quad\text{by 5.2}\\
                             &\simeq 0.
\end{align*}
Thus $\H_\ast((\L D_{B/A})_{\mathfrak{p}})=\H_\ast(\L D_{B/A})_{\mathfrak{p}}=0$ as as $\mathfrak{p}$ is an arbitrary
prime ideal of $B$, we have $\L D_{B/A}\simeq 0$.

Now suppose $B$ smooth over $A$. As $\H_0(\L D_{B/A})\simeq D_{B/A}$ (3.8) there is a canonical map of
projective simplicial $B$-modules $\L D_{B/A}\to cD_{B/A}$. To prove this is an isomorphism we reduce by
...
\[
  \begin{tikzcd}
    \L D_{P/A}\otimes_P B\ar[r,"\sim"]\ar[d,"\sim"'] & \L D_{B/A}\ar[d]\\
    D_{P/A}\otimes_P B\ar[r,"\sim"] & cD_{B/A}
  \end{tikzcd}
\]
...
SGA, 1960--1961, III, 2--3. If also $D_{B/A}=0$, then $B$ is {\'e}tale over $A$.
\end{proof}
...
\[
  0\lra\D_1(B/A,M)\lra I/I^2\otimes_B M\lra D_{P/A}\otimes_P M\lra D_{B/A}\otimes_B M\lra 0
\]
...
\[
  0\lra\D_1(R/k)\lra I/I^2\xrightarrow{\ \delta\ }D_{P/k}\otimes_P R\lra D_{R/k}\lra 0
\]
...
\[
  J/J^2\simeq D_{P/A}\otimes_P B\simeq\L D_{B/A}.\tag{6.4}
\]
...
\[
  E_{pq}^2=\H_{p+q}(S_q^B\L D_{B/A})\Lra\Tor_{p+q}^A(B,B)
\]
...
$\mu(x{\otimes}y)\in J_{p+q}^2$, where $\mu:Q\otimes Q\to Q$ is the
